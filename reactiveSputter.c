...
int main()
  {
    ...
    fprintf(ov, "[p_N]=Pa F theta_1 theta_2 q_ts q_cs q_ps [q_0]=J/s Y\n");
	...
	//1.0 / (1.0 + (J*S_N/e)/(2.0*alpha_t*F));         

	//equation A2' for chamber wall and substrate fractional coverage theta_2:
        theta_two = (2.0*e*F*alpha_c+J*(A_t/A_c)*theta_one*S_N)/(2.0*e*F*alpha_c+J*(A_t/A_c)*(S_M+S_N*theta_one-S_M*theta_two));
	//((e/J)*2.0*alpha_c*F*(A_c/A_t) + S_N*theta_one) / ((e/J)*2.0*alpha_c*F*(A_c/A_t) + S_N*theta_one + S_M*(1.0 - theta_one));       
